...
class Ui_MainWindow(QtWidgets.QWidget):
    def setupUi(self, MainWindow):
        print('SETTING UP UI')
        vars, self.varsArray = self.readVars()
        self.calc = Calculate({}, vars)
        ...
        self.store = QtWidgets.QPushButton(self.gridLayoutWidget)
        self.store.setObjectName("store")
        self.store.clicked.connect(self.forStore)
        self.gridLayout.addWidget(self.store, 0, 3, 1, 1)
        ...
        self.ac = QtWidgets.QPushButton(self.gridLayoutWidget)
        self.ac.setObjectName("ac")
        self.ac.clicked.connect(self.forAc)
        self.gridLayout.addWidget(self.ac, 0, 4, 1, 1)
        ...
        self.ac0 = QtWidgets.QPushButton(self.gridLayoutWidget_2)
        self.ac0.setObjectName("ac0")
        self.gridLayout_2.addWidget(self.ac0, 3, 1, 1, 1)
        ...
        self.naturalLog = QtWidgets.QPushButton(self.gridLayoutWidget_2)
        self.naturalLog.setObjectName("naturalLog")
        self.naturalLog.clicked.connect(lambda: self.refreshDisplay('ln('))
        self.gridLayout_2.addWidget(self.naturalLog, 1, 5, 1, 1)
        ...
        self.ac3 = QtWidgets.QPushButton(self.gridLayoutWidget_2)
        self.ac3.setObjectName("ac3")
        self.gridLayout_2.addWidget(self.ac3, 3, 4, 1, 1)
    ...
    def setPressed(self, val):
        self.varStoragePressed = val

    def drawVarsScreen(self):
        ...
        pressed = None
        a = QPushButton("A")
        a.clicked.connect(lambda: self.setPressed('a'))
        b = QPushButton("B")
        b.clicked.connect(lambda: self.setPressed('b'))
        c = QPushButton("C")
        c.clicked.connect(lambda: self.setPressed('c'))

        d = QPushButton("D")
        d.clicked.connect(lambda: self.setPressed('d'))

        e = QPushButton("E")
        e.clicked.connect(lambda: self.setPressed('e'))

        # Create spare labels
        label_1 = QLabel("Spare label 1")
        label_2 = QLabel("Spare label 2")
        label_3 = QLabel("Spare label 3")
        label_4 = QLabel("Spare label 4")
        label_5 = QLabel("Spare label 5")

        msg.setInformativeText("{}\n{}\n{}\n{}\n{}".format(
            label_1.text(), label_2.text(), label_3.text(), label_4.text(), label_5.text()))

        # Add custom buttons to the message box
        msg.addButton(a, QMessageBox.AcceptRole)
        msg.addButton(b, QMessageBox.AcceptRole)
        msg.addButton(c, QMessageBox.AcceptRole)
        msg.addButton(d, QMessageBox.AcceptRole)
        msg.addButton(e, QMessageBox.AcceptRole)

        # Set the layout direction to be vertical
        # msg.setLayoutDirection(Qt.LeftToRight)

        msg.exec_()
        # Set the informative text of the message box to include the labels
        # return msg, [a, b, c, d, e]

    def forStore(self):
        self.drawVarsScreen()
        if self.calc.lastCalculated != None:
            self.calc.vars[self.varStoragePressed] = self.calc.lastCalculated
            # write vars
            pos = 97-ord(self.varStoragePressed)
            self.varsArray[pos] = self.calc.lastCalculated
            string = ''
            for i in self.varsArray:
                string += f'{i},'
            string=string[:-1]
    ...
    def readVars(self):
        ...
        print(vars)
        varDict = {}
        char = 97  # character code for 'a'
        for var in vars:
            print(var)
            varDict[chr(char)] = float(var)
            char += 1
        return varDict, vars
    ...
    def rightArrow(self):
        if self.flags['fractNum'] == True:
            ...
            self.expressionForCalc += ')/('
            # used for calculations but shouldnt be put in the canvas
            self.expressionStack.push([')/('])
            self.flags['fractNum'] = False
            # self.flags['fractDenom'] = True
        elif self.flags['fractDenom'] == False:
            self.expressionForCalc += ')'
            # used for calculations but shouldnt be put in the canvas
            self.expressionStack.push([')'])
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.ac0.setText(_translate("MainWindow", "pb1"))
        ...
        self.naturalLog.setText(_translate("MainWindow", "ln"))
        ...
        self.ac3.setText(_translate("MainWindow", "pushButton"))
